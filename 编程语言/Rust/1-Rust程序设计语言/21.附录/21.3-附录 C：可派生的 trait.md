...
- [附录 C：可派生的 trait](#附录-c可派生的-trait)
  - [用于程序员输出的 Debug](#用于程序员输出的-debug)
  - [等值比较的 PartialEq 和 Eq](#等值比较的-partialeq-和-eq)
  - [次序比较的 PartialOrd 和 Ord](#次序比较的-partialord-和-ord)
  - [复制值的 Clone 和 Copy](#复制值的-clone-和-copy)
  - [固定大小的值到值映射的 Hash](#固定大小的值到值映射的-hash)
  - [默认值的 Default](#默认值的-default)
...
`Clone` trait 可以明确地创建一个值的 **深拷贝（deep copy）**，复制过程可能包含任意代码的执行以及堆上数据的复制。查阅第四章 “变量与数据交互的方式（二）：克隆” 以获取有关 `Clone` 的更多信息。
